...
// DecodeDeletePersonRequest returns a decoder for requests sent to the user
// deletePerson endpoint.
func DecodeDeletePersonRequest(mux goahttp.Muxer, decoder func(*http.Request) goahttp.Decoder) func(*http.Request) (*user.DeletePersonPayload, error) {
	return func(r *http.Request) (*user.DeletePersonPayload, error) {
		var payload *user.DeletePersonPayload
		var (
			id  int32
			err error
		...
		payload = NewDeletePersonPayload(id)
	...
	return func(ctx context.Context, w http.ResponseWriter, v error) error {
		...
		case "internal_error":
			...
			if formatter != nil {
				...
				body = NewDeletePersonInternalErrorResponseBody(res)
			}
			w.Header().Set("goa-error", res.GoaErrorName())
			w.WriteHeader(http.StatusInternalServerError)
			return enc.Encode(body)
